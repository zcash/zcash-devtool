        ...
        .update_transparent_with(|mut updater| {
            ...
            let inputs_to_update = updater
                ...
                .filter_map(|(index, input)| {
                    ...
                    .and_then(|script| {
                        match script {
                            standard::ScriptKind::PubKeyHash { hash } => {
                                TransparentAddress::PublicKeyHash(hash) == p2pkh_addr
                            }
                            standard::ScriptKind::MultiSig { pubkeys, .. } => {
                                pubkeys.iter().any(|pk| pk == &pubkey_bytes)
                            }
                            standard::ScriptKind::PubKey { data } => data == pubkey_bytes,
                            _ => false,
                        }
                        .then_some(index)
                    })
