...
impl Command {
    pub(crate) async fn run(self, wallet_dir: Option<String>) -> Result<(), anyhow::Error> {
        ...
        if !pczt.transparent().inputs().is_empty() {
            ...
            for (index, (hash_type, redeem_script, script_pubkey, value, bip32_derivation)) in
                transparent_inputs.iter().enumerate()
            {
                println!(
                    "- {index}: {} zatoshis{}, {}",
                    value.into_u64(),
                    match (&config, standard::solver(script_pubkey)) {
                        (Some(config), Some(standard::ScriptKind::PubKeyHash { hash })) => format!(
                            " from {}",
                            TransparentAddress::PublicKeyHash(hash).encode(&config.network())
                        ),
                        (Some(config), Some(standard::ScriptKind::ScriptHash { hash })) => format!(
                            " from {}",
                ...
                match standard::solver(redeem_script.as_ref().unwrap_or(script_pubkey)) {
                    Some(script) => match script {
                        standard::ScriptKind::PubKeyHash { .. } => {
                            println!("  Pay-to-PubKey-Hash (P2PKH)");
                        }
                        standard::ScriptKind::ScriptHash { .. } => {
                            // This case should never occur; `redeem_script` is only
                            // omitted from P2PKH inputs of PCZTs, and P2SH-in-P2SH does
                            // not make sense.
                            println!("  Pay-to-Script-Hash (P2SH)");
                        }
                        standard::ScriptKind::MultiSig { required, pubkeys } => {
                        ...
                        standard::ScriptKind::NullData => println!("  Null data (OP_RETURN)"),
                        standard::ScriptKind::PubKey { .. } => println!("  Pay-to-PubKey (P2PK)"),
                    },
            ...
            Some(tx_data) => {
                ...
                if matches!(tx_data.version(), TxVersion::V5) {
                    ...
                    if tx_data.transparent_bundle().is_some() {
                        ...
                        for (index, (hash_type, redeem_script, script_pubkey, value, _)) in
