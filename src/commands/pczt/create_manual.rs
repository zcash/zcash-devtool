...
impl Coin {
    ...
    fn outpoint(&self) -> anyhow::Result<OutPoint> {
        let hash: [u8; 32] = {
            let mut bytes = hex::decode(&self.hash)?;
            bytes.reverse();
            bytes
                .as_slice()
                .try_into()
    ...
    fn coin(&self) -> anyhow::Result<Option<TxOut>> {
        self.value
            ...
            .map(|(value, script_pubkey)| {
                ...
                let script_pubkey = script::PubKey::from_bytes(&hex::decode(script_pubkey)?)
                    .map_err(|e| anyhow!("{e:?}"))?
                    .0;
    ...
    fn spend_info(&self) -> anyhow::Result<SpendInfo> {
        match (&self.pubkey, &self.redeem_script) {
            ...
            (None, Some(script_hex)) => {
                ...
                let (redeem_script, rest) =
                    script::PubKey::from_bytes(&script_bytes).map_err(|e| anyhow!("{e:?}"))?;
                if rest.is_empty() {
                    Ok(SpendInfo::P2sh { redeem_script })
                } else {
                    Err(anyhow!("redeem_script has trailing bytes"))
                }
            }
        }
    }
